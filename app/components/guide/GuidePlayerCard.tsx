...
interface GuidePlayerCardProps {
	card: Card;
	player: string;
	clickable: boolean;
	cardClass: string;
...
export const GuidePlayerCard = ({ card, player, clickable, cardClass, selectedCard }: GuidePlayerCardProps) => {
	...
	return (
		<>
			{ isReady ? (
				...
				<div className="card flex justify-between h-[15vw] landscape:h-[5vw] landscape:short:h-[10vw] landscape:my-2 landscape:md:h-[7vw] landscape:lg:h-[10vw] max-h-[12vh] landscape:xl:h-[8vw] xtall:max-w-screen-xs:h-[20vw] tall:md:h-[10vw] rounded-sm border border-neutral-400 shadow-md shadow-slate-500 bg-gray-700 aspect-video first:basis-full">
				</div>
			)}
		</>
	);
