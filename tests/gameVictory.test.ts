...
test.describe('game ends when a victory condition is met', () => {
	...
	test.describe('opponent master is defeated', () => {
		...
		test('a victory modal is displayed with Exit Game and Play Again buttons', async ({ page }) => {
			...
			await expect(page.getByRole('heading', { name: 'Congratulations! You won the game!' })).toBeVisible();
		...
		test.describe('when the Play Again button is clicked', () => {
			...
			test('the board is refreshed and new cards are dealt', async ({ page }) => {
				...
				await expect(page.locator("#space-1.student, #space-2.student, #space-3.student, #space-4.student, #space-5.student")).toHaveCount(4);
				...
				await expect(page.locator(".space.red")).toHaveCount(5);
				await expect(page.locator(".space.not-highlighted")).toHaveCount(25);
